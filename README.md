...
Enterprise applications are commonly layered and typically rely on many backend systems, each of which defines its own invocation contract and data transfer objects (DTOs). Integration with the backend systems relies in part on autogenerated code which commonly does not declare interfaces or virtual methods, which limits customization. An example of such integration is the following ```BlogService``` and a corresponding ```Blog``` data transfer object (DTO) which would typically be used in several places of a particular application layer:
...
There may be many other such services, each using custom DTOs. Directly using services in a particular application layer will result in many direct software dependencies and increased effort for developer, who will have to understand service specific DTO details at all times. Suppose that instead of the described approach we extract the invocation contract of a generated service into an interface using layer specific DTO and also have automated means to translate the calls to this custom interface to target invocation contract and back; this would remove direct software dependencies and also alleviate developer's efforts, who will now only have to understand an application layer specific DTO - mappings between this DTO and service specific DTO only has to be defined once, possibly by some other person or team. Having a single common DTO within a particular application layer thus simplifies development, enforces software development practices and increases software reliability by delegating data mapping to well defined software components. AdapterInterceptor is designed for the described approach and enables using custom DTOs for target invocation.

 To illustrate the idea, suppose that with the autogenerated code, or at some other place, we define the following custom interface for the ```BlogService``` example:
...
Furthermore, in a particular application layer there exists a custom DTO, for example the following one which is basically the same as the original but has some data binding attributes added:
...
The actual target invocation and input and return value is taken care of by the AdapterInterceptor library and our custom type mapping code.

### Usage

AdapterInterceptor must be used together with the [Castle DynamicProxy](https://www.nuget.org/packages/Castle.Core/). There are several constructor variants available but generally the following variant should typically be used: ```AdapterInterceptor<TTarget, TSource1, TDestination1, ...>```

```TSource1``` and ```TDestination1``` are types to which the mapping should apply. Support for reverse mapping as well as mappings and reverse mappings of common collection variants, e.g. ```TSource[]``` and ```IList<TDestination>``` is implicitly assumed. AdapterInterceptor constructor accepts an ```IAdapterMapper``` which will actually perform type mapping and which must support the assumptions if those are encountered during invocation. A ```DefaultAdapterMapper``` is provided and it uses AutoMapper:
...
**AdapterInterceptor must always be the last interceptor in the Castle DynamicProxy interceptor order.**
...
Note that invocation and return values may be enumerated by the logger implementation when logging is enabled.
