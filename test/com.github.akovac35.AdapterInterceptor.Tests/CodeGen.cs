...
{supportedPairs}
        ...
        [Test]
        public void Generate_GenerateAdapter_Variants()
        {
            string variants = "";
...
{ description}
        ...
        public static TAdapterInterface GenerateAdapter<TAdapterInterface, TTarget, TSource1, TDestination1{type}>(this TTarget target, IAdapterMapper adapterMapper, ILoggerFactory? loggerFactory = null)
            where TAdapterInterface : class
            where TTarget : notnull
        {{
            var adapterInterceptorTmp = new AdapterInterceptor<TTarget, TSource1, TDestination1{type}>(target, adapterMapper, loggerFactory);
            var proxyGenerator = new ProxyGenerator();
            var adapter = proxyGenerator.CreateInterfaceProxyWithoutTarget<TAdapterInterface>(adapterInterceptorTmp);
            return adapter;
        }}";
                variants += variant + Environment.NewLine;
            }

            _logger.LogInformation(variants);
        }

        [Test]
        public void Generate_AddAdapter_Variants()
        {
            string variants = "";
...
{ description}
        ...
        public static IServiceCollection AddAdapter<TAdapterInterface, TTarget, TSource1, TDestination1{type}>(this IServiceCollection services, Func<IServiceProvider, TTarget> targetFactory, Func<IServiceProvider, IAdapterMapper> adapterMapperFactory, ServiceLifetime serviceLifetime = ServiceLifetime.Scoped, bool useLoggerFactory = true)
            where TAdapterInterface : class
            where TTarget : notnull
        {{
            ...
            {{
                ...
                var adapter = target.GenerateAdapter<TAdapterInterface, TTarget, TSource1, TDestination1{type}>(adapterMapper, loggerFactory);
                return adapter;
            }}, serviceLifetime);
